...
export function PeopleDirectory({ people }: { people: { id: string; name: string; group: string }[] }) {
  ...
  return <div className="people-directory"><label htmlFor="people-search">Find a person</label><input id="people-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Name or connection…" />
    ...
    <div className="notebook-cards people-cards">{visible.map((person) => <Link id={person.id} key={person.id} href={`/library/characters/${person.id}`}>{characterPortraits[person.id] && <div className="people-portrait"><Image src={characterPortraits[person.id].src} alt="" fill sizes="(max-width: 600px) 90vw, (max-width: 900px) 45vw, 280px" className="object-cover" /></div>}<div className="people-card-copy"><span>{person.group}</span><h2>{person.name}</h2><p>History · motivations · relationships →</p></div></Link>)}</div>
